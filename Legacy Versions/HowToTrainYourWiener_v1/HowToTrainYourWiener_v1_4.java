...
public class HowToTrainYourWiener_v1_4 {
    ...
    private static int dogBaseFontSize = 90;
    ...
    private static void buildUI() {
        ...
        petBtn.addMouseListener(new MouseAdapter() {
            @Override public void mousePressed(MouseEvent e) {
                // Spawn treat particle at exact mouse position in frame coords
                ...
                spawnTreatParticle(p.x, p.y, "+" + gain + " 🍖");
            }
        });
    ...
    private static void animateClick(JButton btn) {
        ...
        clickAnimStep = 0;
        Color[] frames = {
            new Color(255, 240, 0),   // yellow flash
            new Color(255, 160, 30),  // orange
            new Color(255, 100, 20),  // red-orange
            new Color(255, 80,  40),  // original
        };
        ...
        clickAnimTimer.addActionListener(e -> {
            if (clickAnimStep < frames.length) {
                ...
                // Jiggle: shift button left/right
                Rectangle r = btn.getBounds();
                int jitter = (clickAnimStep % 2 == 0) ? 3 : -3;
                btn.setBounds(r.x + jitter, r.y, r.width, r.height);
                clickAnimStep++;
            } else {
                // Restore original position & colour
                Rectangle r = btn.getBounds();
                btn.setBounds(15, 435, 185, 42);
    ...
    /**
     * Spawns a floating "+N 🍖" label that rises and fades — Cookie Clicker style.
     * Multiple particles spread out slightly so they don't stack perfectly.
     */
    private static void spawnTreatParticle(int x, int y, String text) {
        if (particleLayer == null) return;

        // Slight random spread so rapid clicks don't all overlap
        int spawnX = x + random.nextInt(40) - 20;
        int spawnY = y;

        JLabel lbl = new JLabel(text);
        lbl.setFont(new Font("Courier New", Font.BOLD, 14));
        lbl.setForeground(new Color(255, 230, 0));
        lbl.setSize(lbl.getPreferredSize());
        lbl.setLocation(spawnX - lbl.getWidth() / 2, spawnY);
        particleLayer.add(lbl);
        particleLayer.repaint();

        // 30 frames: rise 2px per frame, fade alpha each step
        int[] frame = {0};
        int totalFrames = 35;
        int[] currentY = {spawnY};
        ...
        t.addActionListener(e -> {
            frame[0]++;
            currentY[0] -= 2;
            // Fade: map frame 0→35 to alpha 255→0
            int alpha = Math.max(0, 255 - (int)(255.0 * frame[0] / totalFrames));
            lbl.setForeground(new Color(255, 230, 0, alpha));
