...
public class HowToTrainYourWiener_v1_13 {
    ...
    private static void buildUI() {
        ...
        MouseAdapter dogPetListener = new MouseAdapter() {
            @Override public void mousePressed(MouseEvent e) {
                ...
                else addLog("> PET: +" + gain + " treats" + (comboCount > 1 ? "  COMBO x" + comboCount : ""));
                updateGame();
                animateDogBounce();
        ...
        staminaBar.setBounds(355, 310, 290, 20);
        ...
        // Stamina bar stretches full width now that REST button is removed
    ...
    private static void startIdle() {
        new Timer(1000, e -> {
            if (gameOver) return;
            // Auto-rest: when stamina hits 20 or below, force rest until full
            if (!isResting && stamina <= 20) {
                isResting = true;
                addLog("> 💤 AUTO-REST: Wiener is exhausted! Resting until stamina is full...");
            }
            if (isResting) {
                stamina = Math.min(MAX_STAMINA, stamina + 10);
                if (stamina >= MAX_STAMINA) {
                    isResting = false;
                    addLog("> ✅ STAMINA RESTORED! Wiener is ready to train again!");
                }
                updateGame();
                return; // skip passive gains and events while resting
            }
